...
而且经常压得很低，甚至满屏漂浮。分两步处理。

**第一步：让字幕本身不怕背景**（收益最大）。默认就是半透明底板样式，任何背景都清晰。
如果之前用的是 `--style outline`，浅色背景上会发灰，换回来：

```bash
<PY> scripts/02_make_srt.py <工作目录> --video video.mp4 --style plate
```

**第二步：逐句避让。** 分析每句的时间区间里底部字幕带是否被画面原有内容占住，
只把确实被占住的句子抬到空位：

```bash
<PY> scripts/00_probe_safe_area.py video.mp4 \
    --segments <工作目录>/segments.json --plan-out <工作目录>/placement.json
```

然后重新生成字幕即可，第 3 步会自动读取 `placement.json`，不需要额外参数。

> **预期管理**：如果画面大字满屏漂浮，往往**根本不存在**干净的空位——实测一条宣传片
> 52 句里只有 2 句躲得开，其余 30 句虽然底部被挡，但画面里任何高度都被挡。
> 这时只能：只出中文字幕（`zh.srt` 占高更小）、用 `--margin-v` 手工调到某个
> 特定段落躲开、或者接受部分遮挡。逐句避让的作用是「**有空位时**才避让」，
> 不是「保证不叠字」。

想看画面各纵向位置的占用情况（纯查看，不改任何文件）：
