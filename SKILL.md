...
字幕默认用**半透明底板**（`--style plate`）：文字坐在一层半透明黑底上，
所以深色、浅色、以及明暗频繁切换的画面都清晰。实测同一句字幕落在纯白背景上，
描边样式会发灰难认，底板样式完全不受影响。不想要底板、只要描边就用
`--style outline`，画面更干净，代价是浅色背景上对比度偏弱。

**画面里本身就有大字排版时**（宣传片、发布会常见，且经常压得很低），
先做逐句避让分析：

```bash
<PY> scripts/00_probe_safe_area.py "<原视频>" \
    --segments <工作目录>/segments.json --plan-out <工作目录>/placement.json
```

它逐句判断「这一句的时间区间里，底部字幕带是否被画面原有内容占住」，
只把确实被占住的句子抬到空位，其余全部贴底——避免字幕频繁上下跳。
之后第 3 步会自动读取 `placement.json`，不需要额外参数。

> 但要有预期：这类素材的画面大字会满屏漂浮，**固定位置无论选哪儿都会有一部分
> 句子撞上**。实测一条动态排版宣传片（176s / 52 句）：下半幅几乎处处有内容，
> 52 句里只有 2 句真正找得到干净的空位。所以这一层避让属于锦上添花，
> **真正兜住可读性的是底板样式**。想彻底不叠字，只能退回纯中文单行字幕
> （`zh.srt`，占高更小），或者把 `--margin-v` 手工调到某个特定段落躲开。
